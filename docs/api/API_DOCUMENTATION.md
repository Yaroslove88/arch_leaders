...
## Retention (Отслеживание активности)
...
```json
{
  "userId": "91500418-d30d-49f3-9af0-0f881d90333b",
  "activityType": "any"
}
```
...
- `activityType` (optional) - Тип активности: `case`, `quest`, `entry`, `trace`, `any` (по умолчанию: `any`)
...
Получить данные о retention пользователя (серии, даты активности).
...
```json
{
  "currentStreak": 5,
  "longestStreak": 10,
  "lastActivityDate": "2026-01-09T00:00:00.000Z",
  "activityDates": ["2026-01-05", "2026-01-06", "2026-01-07", "2026-01-08", "2026-01-09"]
}
```

### `GET /retention/:userId/risk`
Проверить, под угрозой ли серия активности пользователя.
...
**Описание полей:**
- `isAtRisk` - серия прервётся через несколько часов (если вчера была активность, но сегодня ещё нет)
- `daysWithoutActivity` - количество дней без активности
- `shouldRemind` - нужно ли показать напоминание (true если daysWithoutActivity >= 3 или isAtRisk)

**Примечание:** Retention данные хранятся в PostgreSQL (таблица `user_retention`). Серия активности рассчитывается на основе последовательных дней с активностью.

### Пример использования Retention API

```bash
# Записать активность пользователя
curl -X POST http://localhost:3001/retention/activity \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "91500418-d30d-49f3-9af0-0f881d90333b",
    "activityType": "any"
...
# Получить retention данные
curl http://localhost:3001/retention/91500418-d30d-49f3-9af0-0f881d90333b

# Проверить риск потери серии
curl http://localhost:3001/retention/91500418-d30d-49f3-9af0-0f881d90333b/risk
```
